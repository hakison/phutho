...
install_projectivy() {
   ...
   $ADB_COMMAND shell pm uninstall --user 0 "$app" >/dev/null 2>&1
    ...
    do
        # Sửa $ADB thành $ADB_COMMAND shell và thêm ngoặc kép "$cmd"
        $ADB_COMMAND shell "$cmd" >/dev/null 2>&1
    done
...
install_X_S_2026() {
   ...
   for app in "${apps[@]}"; do
    ...
    # SỬA LỖI TREO: Chạy reboot trong background để script tiếp tục ngay lập tức
    $ADB_COMMAND reboot & 
    PID=$!
    sleep 2
    kill $PID >/dev/null 2>&1 # Ngắt lệnh reboot để terminal không bị treo đợi phản hồi
    ...
    while true; do
       # Thử kết nối lại
      $ADB_COMMAND disconnect >/dev/null 2>&1
      RESULT=$($ADB_COMMAND connect "$DEVICE_IP")
    
     # Kiểm tra xem thiết bị đã sẵn sàng chưa (trạng thái 'device')
     # Lưu ý: 'grep -w device' để tránh nhầm với 'offline' hoặc 'unauthorized'
    ...
    # Đợi thêm 2 giây để hệ thống ổn định trước khi chạy shell
    sleep 2
