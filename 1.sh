...
# 1. Cài đặt Projectivy Launcher và các app đi kèm
install_projectivy() {
    echo -e "${CYAN}⚙️ BƯỚC 2: ĐANG ÉP XUNG ĐỔI NGÔN NGỮ TIẾNG VIỆT TOÀN HỆ THỐNG...${NC}"
    
    # 🔴 GIẢI PHÁP SỬA LỖI GITHUB: Định nghĩa biến sạch, bọc chặt tham số vi-VN
    local lang_vi="vi-VN"
    local country_vn="VN"
    local zone_bk="Asia/Bangkok"

    # Gửi trực tiếp chuỗi lệnh sạch, loại bỏ hoàn toàn nguy cơ dính ký tự ẩn \r từ file GitHub
    ...
    # Ép phân vùng hệ thống AndroidTV nhận diện lại cấu hình tiếng Việt
    ...
    # Các lệnh cấu hình hệ thống bổ trợ khác
    for cmd_sys in \
    ...
    do
        $ADB_COMMAND shell "$cmd_sys" >/dev/null 2>&1
    done

    echo -e "${GREEN}✅ Đã ép mã hóa ngôn ngữ tiếng Việt gốc thành công!${NC
    ...
    # ĐÃ SỬA: Viết trọn vẹn trên 1 dòng để vòng lặp Bash không bỏ sót bất kỳ gói nào bên dưới
    ...
    for app in $apps; do
        # 1. Buộc dừng tiến trình app trước khi tác động
        $ADB_COMMAND shell am force-stop "$app" >/dev/null 2>&1
        # 2. Vô hiệu hóa ứng dụng ngầm
        $ADB_COMMAND shell pm disable-user --user 0 "$app" >/dev/null 2>&1
        # 3. Gỡ bỏ vật lý khỏi phân vùng user 0
    ...
    # Tạo danh sách mảng lệnh chuẩn hóa (Đã sửa lại Activity chuẩn của Projectivy)
    local list_xs_a14=(
        "package clear-app-profiles com.mitv.tvhome"
        ...
        "package set-home-activity com.spocky.projengmenu/.MainActivity"
    )
    ...
    echo -e "\n${YELLOW}⚡ BƯỚC 3: ĐANG BIÊN DỊCH TỐC ĐỘ CAO CHO LAUNCHER...${NC}"
    # Ép TV biên dịch mã nhị phân tốc độ cao cho Projectivy để khởi động phím Home không bị trễ
    ...
    echo -e "\n${YELLOW}🔄 BƯỚC 4: GHI DỮ LIỆU BẢO MẬT VÀ ĐỒNG BỘ Ổ CỨNG TV...${NC}"
    ...
    # Ngắt kết nối an toàn để iSH không bị treo tiến trình ngầm khi reboot TV
...
install_X_S_2026() {
    ...
    # Gọi hàm sao chép ảnh nền bên dưới
    copy_wallpapers
    ...
    # Định nghĩa danh sách các lệnh cấu hình ADB chuẩn (Không dùng phân tách dấu | dễ lỗi)
    local list_xs_a14=(
        "package clear-app-profiles com.mitv.tvhome"
        "pm grant com.mitv.shareds android.permission.WRITE_SECURE_SETTINGS"
        "pm grant com.mitv.shareds android.permission.CHANGE_CONFIGURATION"
        ...
        "settings put secure enabled_accessibility_services com.mitv.shareds/com.mitv.shareds.HomeService"
        "settings put secure accessibility_enabled 1"
        "package set-home-activity com.spocky.projengmenu/.ui.home.MainActivity"
    )

    echo -e "${YELLOW}⚙️ Đang thực thi nạp quyền cấu hình nâng cao vào TV...${NC}"

    # Chạy vòng lặp duyệt qua mảng lệnh một cách chính xác
    for cmd in "${list_xs_a14[@]}"; do
        # Gửi lệnh trực tiếp qua adb shell ẩn log lỗi hệ thống
        $ADB_COMMAND shell "$cmd" >/dev/null 2>&1
    done

    # Xóa dòng trạng thái cũ bằng cách ghi đè khoảng trắng và in thông báo hoàn tất
    printf "\r%115s\r" " "
    echo -e "${GREEN}✅ Đã hoàn tất toàn bộ cấu hình hệ thống nâng cao!${NC}"
    
    echo -e "\n${YELLOW}⚡ Đang tối ưu hóa riêng Projectivy Launcher...${NC}"
    # ĐÃ SỬA: Đồng bộ hóa gói ứng dụng thành com.spocky.projengmenu chuẩn xác
    $ADB_COMMAND shell cmd package compile -m speed -f com.spocky.projengmenu >/dev/null 2>&1
    $ADB_COMMAND shell settings put global install_non_market_apps 1 >/dev/null 2>&1
    ...
    # ĐÃ SỬA: Ngắt kết nối adb an toàn trước khi khởi động lại để chặn hoàn toàn lỗi treo RAM trên iSH
    $ADB_COMMAND shell reboot >/dev/null 2>&1 &
    sleep 1
    $ADB_COMMAND disconnect >/dev/null 2>&1
